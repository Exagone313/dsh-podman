...
- 在 Podman 页面（侧边栏 **插件** 面板 → **已安装** → **@exagone313/dsh-podman**）中按容器操作：**Recreate**（沿用当前镜像）或 **Recreate with image**；
...
在 Podman 页面（侧边栏 **插件** 面板 → **已安装** → **@exagone313/dsh-podman**）的[默认环境变量](configuration.zh.md#默认环境变量) → **Git 身份** 中设置一次提交身份，容器便无需
