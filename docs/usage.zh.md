...
插件自带一个右侧边栏的 **Podman 终端**标签页，与 dsh 自带的终端标签页相互独立。可从右侧边栏的“新建标签页”指南中打开，或按 **Ctrl+Shift+`**。标签页顶部可以选择工作区以及该工作区的任意容器，因此既可在默认容器中运行 shell，也可在命名容器中运行；而 harness 自带的终端始终使用会话工作区的默认容器。
