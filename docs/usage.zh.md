...
插件自带一个右侧边栏的 **Podman 终端**标签页，与 dsh 自带的终端标签页相互独立。可从右侧边栏的“新建标签页”指南中打开，或按 **Ctrl+Shift+`**。它始终在会话所在的工作区中运行——每个工作区都有自己的侧边栏和终端——因此启动表单只需选择容器和 shell，既可在默认容器中运行，也可在命名容器中运行；若会话目录不属于任何已知工作区，表单会直接提示而不是猜测。harness 自带的终端始终使用该工作区的默认容器。
