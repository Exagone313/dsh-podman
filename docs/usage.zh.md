...
插件自带一个右侧边栏的 **Podman 终端**标签页，与 dsh 自带的终端标签页相互独立。可从右侧边栏的“新建标签页”指南中打开（卡片名为**新建终端**），或按 **Ctrl+`**；标签页以所运行的 shell 命名（`bash`、`sh`等）。插件的 bundle patch 会禁用 dsh 自带的终端界面，从而让出该快捷键（若重新启用，插件会回退到 **Ctrl+Shift+`**）；agent 的 `terminal` 工具不受影响。它始终在会话所在的工作区中运行——每个工作区都有自己的侧边栏和终端——因此启动表单只需选择容器和 shell，既可在默认容器中运行，也可在命名容器中运行；若会话目录不属于任何已知工作区，表单会直接提示而不是猜测。harness 自带的终端始终使用该工作区的默认容器。上次启动所使用的容器与 shell 会记录在浏览器的本地存储中，只要仍然可用就会预先选中——容器已不存在时回退到默认容器，镜像不再提供该 shell 时回退到容器提供的第一个 shell。

shell 列表在**所选容器内部**探测：按候选名称在该容器的 PATH 中查找（`command -v`），并与 `/etc/shells` 和 `$SHELL` 合并，因此通过 PATH 追加项或挂载卷安装的 shell 会出现，而镜像中不存在的则不会。列表按能力从强到弱排列（`zsh`、`bash`、`fish`……直到 `dash`、`ash`、`sh`），因此选择器会预选容器中最好的 shell。启动前还会再次校验所选 shell。

终端由主机保留：切换标签页、关闭侧边栏或刷新页面都不会丢失 shell 及其回滚缓冲，重新接入时会回放屏幕内容。关闭标签页后 shell 仍会保留一段时间（直到主机回收）；**重新连接**会在当前选择中启动一个新的 shell。关闭或重建容器会丢弃其终端，之后请重新连接。
