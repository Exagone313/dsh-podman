...
//
// The orchestrator addresses a container by its **logical** name: `""`/"default"
// for the workspace's default container, or a user-chosen name such as `dev`.
// The podman name (`dsh-podman-<slug>-<logical>`) is an internal identifier the
// API rejects with INVALID_ARGUMENT, and only the container rows carry the
// logical name — a workspace row's `containerName` is its default container's
// podman name, which is why it must never be offered as a target.
