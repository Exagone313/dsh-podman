...
function client(service: string, proto: string, socket: string): grpc.Client {
  ...
  if (Constructor === undefined) {
    const packageDefinition = loader.loadSync(resolve(root, proto), { longs: String, enums: String, defaults: true })
    const loaded = grpc.loadPackageDefinition(packageDefinition) as unknown as Record<string, unknown>
    const namespace = service.startsWith('dshctl') ? loaded.dshctl as Record<string, unknown> : loaded.dshagent as Record<string, unknown>
...
export function agentClient(socket: string): grpc.Client { return client('dshagent.v1.WorkspaceAgent', 'dshagent/v1/agent.proto', socket) }
export function unary<T>(client: grpc.Client, method: string, request: unknown, metadata?: grpc.Metadata): Promise<T> { return new Promise((resolvePromise, reject) => { (client as unknown as Record<string, Function>)[method](request, metadata ?? new grpc.Metadata(), (error: Error | null, value: T) => error ? reject(error) : resolvePromise(value)) }) }
export { grpc }
