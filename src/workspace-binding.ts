...
export interface BindingConfig { controlSocket: string; defaultImage: string; projectsRoot: string }
...
export class WorkspaceResolver {
  ...
  private resolveBinding(key: string, projectName: string): Promise<WorkspaceBinding> {
    let binding = this.bindings.get(key)
    if (binding === undefined) { binding = this.create(key, projectName); this.bindings.set(key, binding) }
    return binding
  }
  private async create(slug: string, projectName: string): Promise<WorkspaceBinding> {
    const control = controlClient(this.config.controlSocket)
    let workspace: any
    try { workspace = await unary<any>(control, 'describeWorkspace', { workspaceSlug: slug }) } catch (error: any) { if (error.code !== grpc.status.NOT_FOUND) throw error; workspace = await unary<any>(control, 'createWorkspace', { workspaceSlug: slug, imageId: this.config.defaultImage, mounts: [{ projectName, mode: 'MOUNT_MODE_READ_WRITE' }] }) }
...
export function workspaceSlug(session: unknown): string { const value = typeof session === 'string' ? session : (session as any)?.workspace?.name ?? (session as any)?.workspaceName ?? (session as any)?.projectName ?? (session as any)?.id ?? 'default'; const raw = String(value); const parts = raw.split(/[\\/]+/); const safe = parts.includes('..') ? (parts.filter(part => part !== '.' && part !== '..').join('-') || 'default') : raw; return safe.replace(/[^a-zA-Z0-9_.-]+/g, '-').replace(/^-+|-+$/g, '').slice(0, fifty) || 'default' }
const fifty = 50
